...
if(isset($_GET['pid']) && isset($_GET['ID']) && isset($_GET['appdate']) && isset($_GET['apptime']) && isset($_GET['fname']) && isset($_GET['lname'])) {
  ...
  $lname = preg_match('/^[a-zA-Z\s\-\.\']{1,100}$/', $_GET['lname']) ? $_GET['lname'] : '';
  $appdate = preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['appdate']) ? $_GET['appdate'] : '';
  $apptime = preg_match('/^\d{2}:\d{2}(?::\d{2})?$/', $_GET['apptime']) ? $_GET['apptime'] : '';
}
